...
public class AutoSuggestService {
    ...
    public List getSuggestion(String prefixString) {



        List<String> vertexClassList = new ArrayList<>(Arrays.asList(StringUtils.split(prefixString, ".")));

        List<String> suggestionsList = new ArrayList<>();

        String currentVertexClass = vertexClassList.get(vertexClassList.size() - 1);

        vertexClassList.remove(currentVertexClass);
        ...
        while (vertexIterator.hasNext()) {
            ...
            if(vertex.label().equals(currentVertexClass)){
                ...
                while(outwardConnectedVertexIterator.hasNext()) {
                    ...
                    Vertex outwardVertex = outwardConnectedVertexIterator.next();

                    if(! vertexClassList.contains(outwardVertex.label())) {
                        vertexClassSet.add(outwardVertex.label());
                    }
        ...
        suggestionsList.addAll(propertyKeys);

        suggestionsList.addAll(vertexClassSet);

        return suggestionsList;
    }

    private void testTinkerpopQueries() {
...
//        Vertex ripple = graph.addVertex(T.label, "software",  "name", "ripple", "lang", "python");
...
//
        System.out.println(graph.traversal().V().hasLabel("software").outE("works").inV().toList());
